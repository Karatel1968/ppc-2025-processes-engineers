...
void UrinOGaussVertDiagMPI::GenerateRandomMatrix(
...
void UrinOGaussVertDiagMPI::CalculateColumnDistribution(
    size_t n, int size,
...
bool UrinOGaussVertDiagMPI::RunImpl() {
  ...
  for (size_t k = 0; k < n; ++k) {
    ...
    if (k >= static_cast<size_t>(start_col) &&
        k < static_cast<size_t>(start_col + local_cols)) {

      int lk = static_cast<int>(k) - start_col;
      for (size_t i = k; i < n; ++i) {
        local_max = std::max(
            local_max,
            std::abs(a_local[i * local_cols + lk]));
      }
    }
    ...
    for (int p = 0; p < size; ++p)
      if (k >= static_cast<size_t>(displs[p]) &&
          k < static_cast<size_t>(displs[p] + counts[p]))
        owner = p;
    ...
    if (rank == owner) {
      ...
      for (int j = 0; j < local_cols; ++j)
        pivot_row[start_col + j] =
            a_local[k * local_cols + j] / pivot;
    }

    MPI_Bcast(pivot_row.data(), static_cast<int>(n + 1),
    ...
    for (size_t i = k; i < n; ++i) {
      ...
      if (k >= static_cast<size_t>(start_col) &&
          k < static_cast<size_t>(start_col + local_cols)) {
        factor = a_local[i * local_cols +
                         (static_cast<int>(k) - start_col)];
      }

      MPI_Bcast(&factor, 1, MPI_DOUBLE, owner, MPI_COMM_WORLD);

      for (int j = 0; j < local_cols; ++j)
        a_local[i * local_cols + j] -=
            factor * pivot_row[start_col + j];
    }
  }
  ...
  MPI_Allreduce(&local_norm, &global_norm,
